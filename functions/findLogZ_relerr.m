function [logZ,NF0_hat] = findLogZ_relerr(F0,NF0,dx,GlobalParams,makePlots,varargin)
% logZ = findLogZ_relerr(F0,NF0,GlobalParams,dx,varargin)
...
options = optimoptions('fmincon','MaxIterations',1e6,'MaxFunctionEvaluations',1e6,'FiniteDifferenceType','central','OptimalityTolerance',1e-3,'StepTolerance',1e-3,'FunctionTolerance',1e-3);%,'Display','none');
...
if isempty(varargin)
    ...
    function residuals = computeRes(logZ,F0,NF0,GlobalParams,z)
        ...
        residuals = log(integrals(:)) - log(NF0(:));
        residuals(NF0<1e-6) = 100;
        residuals(integrals<1e-6) = 100;
